...
export class StatusComponent {
  ...
  constructor(
    ...
    this.unsubscribeCurrentUserData = authService.currentUser
      .pipe(
  ...
  getData(uid: string) {
    this.unsubscribeUserData = combineLatest([
      this.fsd
        .getCollection(uid)
        .pipe(map((data: any[]) => this.getTransformedData(data))),
      this.bds.getCurrentBitcoinPrice(),
    ])
      .pipe(
        map(([transformedData, bitcoinPrice]) =>
          this.getCombinedData(transformedData, bitcoinPrice)
        ),
        catchError((err: Error) => {
          this.notificationService.error(
            `Something went wrong! ${err.message}`
      ...
      .subscribe(
        (combinedData: CombinedData | never[]) =>
  ...
  private getCombinedData(
    ...
    const stackValue = this.utility.formatNumber(
      bitcoinPrice.EUR * transformedData.stack,
      2
    );
    const unrelizedGain = this.utility.formatNumber(
      stackValue - transformedData.cost,
      2
    );
    const unrelizedGainInPercent = this.utility.formatNumber(
      (unrelizedGain / transformedData.cost) * 100,
      2
    );
    return {
