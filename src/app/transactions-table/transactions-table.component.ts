...
export class TransactionsTableComponent {
  ...
  getDataForTable(uid: string) {
    this.unsubscribeTransactionData = combineLatest([
      ...
      .pipe(
        map(([transactionData, bitcoinPrice]) =>
          transactionData.map((transaction: any) => ({
            ...
            value: this.utilityService.getCurrentValue(
              transaction,
              bitcoinPrice as BitcoinPrice
            ),
          }))
        ),
  ...
  refreshTransactions() {
    this.transactions = this.transactionData
      .map((transaction, i) => ({ newId: i + 1, ...transaction }))
      .slice(
  ...
  openTransaction(data: any) {
    this.modalService.editTransaction(data);
